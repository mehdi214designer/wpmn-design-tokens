...
4. **Copy:** edit the text layers. Base font is **Manrope**. If a layer's font will not load (e.g. Axiforma), set its `fontName` to Manrope at the matching weight, then set the text. Wrap font loads in try/catch. Hide unwanted layers with `visible = false` (don't blank them).
