# Build Spec: WPMN Section — Infinite Marquee

Continuous auto-scrolling logo band via duplicated track + translateX keyframes (28s loop). Pauses on hover, honors reduced motion.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/infinite-marquee/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-infinite-marquee` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-infinite-marquee">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
