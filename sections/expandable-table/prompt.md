# Build Spec: WPMN Section — Expandable Table

Comparison table whose rows expand inline (max-height transition) to reveal a 3-fact detail grid. Header hides on mobile.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/expandable-table/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-expandable-table` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-expandable-table">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
