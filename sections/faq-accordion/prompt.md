# Build Spec: WPMN Section — FAQ Accordion

Centered section header + 800px accordion list. One item open at a time, plus icon rotates 45deg to an X, border highlights in brand color, max-height transition 250ms.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/faq-accordion/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-faq-accordion` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-faq-accordion">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
