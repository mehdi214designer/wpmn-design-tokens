# Build Spec: WPMN Section — Blog Cards

3-column article cards: 16:9 cover, category + read time meta, title, excerpt. Whole card is the link with hover lift.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/blog-cards/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-blog-cards` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-blog-cards">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
