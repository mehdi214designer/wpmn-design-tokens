# Build Spec: WPMN Section — Product Showcase

E-commerce best picks grid: large featured product card spanning two rows + 2x2 grid of smaller cards. Soft grey card fills, image zoom + lift on hover, staggered fade-up reveal on scroll, View All pill. Product images are hotlinked from Unsplash; swap for real product PNGs in production.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/product-showcase/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-product-showcase` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-product-showcase">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
