# Build Spec: WPMN Section — Work Portfolio Shade

Pinned window-shade portfolio: project rows collapse from full height to a thin strip sequentially as the page scrolls (1000px of scroll per item), with image hover zoom, an ink counter pill, and a See More button with text-slide + corner-arrow hover. Falls back to natural flow below 1024px. Keep the scroll mechanics (vw-based geometry, sticky offsets, rAF loops) exactly as in the reference; only colors, typography and spacing are tokenized.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/work-portfolio/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-work-portfolio` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-work-portfolio">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
