# Build Spec: WPMN Section — Competitor Comparison

Us-vs-them matrix: highlighted Us column, success checks and error crosses, mixed text cells, CTA below.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/comparison-table/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-comparison-table` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-comparison-table">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
