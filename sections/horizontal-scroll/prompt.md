# Build Spec: WPMN Section — Horizontal Scroll

300vh tall wrapper pins a viewport; vertical scroll progress translates a horizontal panel track. Falls back to native overflow for reduced motion.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/horizontal-scroll/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-horizontal-scroll` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-horizontal-scroll">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
