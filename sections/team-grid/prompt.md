# Build Spec: WPMN Section — Team Grid

4-column people cards: circular avatar, name, role, social icon links. Hover lifts shadow.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/team-grid/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-team-grid` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-team-grid">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
