# Build Spec: WPMN Section — Multi-Column Cards

Classic 3-column feature cards (guideline grid #3, 32px gutter): icon tile, heading, body. Hover lift.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/multi-column-cards/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-multi-column-cards` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-multi-column-cards">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
