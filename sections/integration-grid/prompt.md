# Build Spec: WPMN Section — Integration Grid

4-column app tiles: logo tile, name, one-line purpose. Hover highlights border in brand color. Browse-all link below.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/integration-grid/section.html`
...
A single self-contained `<section class="wpmn-sec-integration-grid">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
