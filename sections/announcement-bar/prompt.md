# Build Spec: WPMN Section — Announcement Bar

Slim dark dismissible bar for promos: bold lead-in, arrow link, close button. Sits above the navbar.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/announcement-bar/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-announcement-bar` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-announcement-bar">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
