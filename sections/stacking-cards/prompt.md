# Build Spec: WPMN Section — Stacking Cards

Cards pin with position:sticky at stepped offsets and stack as the page scrolls. Pure CSS, no scroll hijack.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/stacking-cards/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-stacking-cards` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-stacking-cards">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
