# Build Spec: WPMN Section — Typographic Hero

Minimalist hero built on massive type: clamp(48px, 8vw, 96px) bold headline with brand accent span, subtext, paired CTAs. No imagery.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/hero-typographic/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-hero-typographic` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-hero-typographic">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
