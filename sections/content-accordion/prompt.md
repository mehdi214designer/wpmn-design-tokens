# Build Spec: WPMN Section — Content Accordion

Two-column disclosure: pinned heading left, numbered single-open accordion right. Max-height transition 250ms, chevron rotates 180deg.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/content-accordion/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-content-accordion` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-content-accordion">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
