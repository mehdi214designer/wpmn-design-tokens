# Build Spec: WPMN Section — Mockup Showcase

Browser-framed screenshot (chrome dots + URL bar) on a dark surface stage, following the guideline mockup rules: 16px frame radius, soft-500 shadow, 32px stage radius.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/mockup-showcase/section.html`
...
A single self-contained `<section class="wpmn-sec-mockup-showcase">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
