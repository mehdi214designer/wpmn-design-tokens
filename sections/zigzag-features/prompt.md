# Build Spec: WPMN Section — Zig-Zag Features

Alternating image/text rows (even rows flip order). Kicker, heading, body, arrow link per row. Stacks image-first on mobile.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/zigzag-features/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-zigzag-features` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-zigzag-features">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
