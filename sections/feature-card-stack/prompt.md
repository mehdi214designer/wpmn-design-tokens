# Build Spec: WPMN Section — Feature Card Stack

Sticky section heading with dark feature cards that stack over it on scroll; each pinned card scales down to 0.94 as the next slides over (rAF-driven, linear with scroll). Cards alternate text/image and carry an arrow-swap button with a brand gradient glow chip. Keep the scroll mechanics (vw-based geometry, sticky offsets, rAF loops) exactly as in the reference; only colors, typography and spacing are tokenized.

The reference implementation is the source of truth. Fetch it and reproduce structure, spacing, and interactions exactly:
`https://raw.githubusercontent.com/mehdi214designer/wpmn-design-tokens/master/sections/feature-card-stack/section.html`
...
- Class scoping: every rule is prefixed with `.wpmn-sec-feature-card-stack` so sections can coexist on one page.
...
A single self-contained `<section class="wpmn-sec-feature-card-stack">` containing a scoped `<style>` block, the markup, and (if the section is interactive) a scoped IIFE `<script>`. No external dependencies, no libraries.
